...
function NewPastePage() {
  ...
  const router = useRouter();const [textEditor, setTextEditor] = useState("");const data = {textEditor: textEditor};

  const [CreatePasteMutation] = useMutation(createPaste) // Blitzjs useMutation Variant -  Issue the Mutation

  const handleSubmit = async (values) => { // handle Submission handler - Submission Form Handler

    const [textEditor, setTextEditor] = useState("") // set useState on textEditor
    const paste = await CreatePasteMutation(values);await router.push(Routes.ShowPastePage({ pasteId: paste.id }));}
