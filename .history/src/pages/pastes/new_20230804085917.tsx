...
function NewPastePage() {
  ...
  const handleSubmit = async (event: { pasteDescription: string; pasteName: string; pasteContent: string } | undefined, editor: any) => {
    const paste = await CreatePasteMutation(data);await router.push(Routes.ShowPastePage({pasteId: paste.id}));console.log("Pressed Button!");
    console.log( { event, editor, data, pasteName } );
  }
