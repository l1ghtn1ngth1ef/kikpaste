...
function NewPastePage() {
  ...
  const handleSubmit = async (event: { pasteDescription: string; pasteName: string; pasteContent: string } | undefined, editor: any) => {

    const paste = await CreatePasteMutation(event);await router.push(Routes.ShowPastePage({pasteId: paste.id}));console.log("Pressed Button!");
    console.log(textEditor);console.log("Values!", event)
    console.log( { event, editor, data } );
  }

  const inputHandler = (_event: any, _editor: any) => {
  }
      ...
      <div>
        <Form onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <CKEditor id="inputText" editor={ClassicEditor}data={textEditor} schema={CreatePasteSchema}
            onReady={(editor) => {
              const data = editor.getData();setTextEditor(data)}}
            onChange={(_event, editor) => {const data = editor.getData();setTextEditor(data);
            }}
            />
          </div>
