...
const handler: BlitzApiHandler = async (req: BlitzApiRequest,
  ...
  try { // try the Database Routine 
    ...
    const data = await db.paste.findUnique({ where: { id: pasteId } }); // if I change this to 2 I get paste 2 in browser so thats good, but it should work like webhook/3 and get paste 3 
